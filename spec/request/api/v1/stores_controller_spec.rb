...
RSpec.describe Api::V1::StoresController, type: :request do
  ...
  context "data attributes" do
    it "#store properties" do
      get "/api/v1/stores", {}, API_V1_HEADERS
  ...
  context "filter" do
    it "filter with wrong search term" do
      get '/api/v1/stores?title=like:title', {}, API_V1_HEADERS
      expect(response.code).to eq('200')
      expect(json).to eq([])
    end

    it "filter with  correct search term" do
      get '/api/v1/stores?title=like:title1', {}, API_V1_HEADERS
      expect(response.code).to eq('200')
  ...
  context "Action" do
    it "#create" do
      post "/api/v1/stores", {store: {title: 'title', city: 'city', street: 'street', spaces_count: 1}}.to_json, API_V1_HEADERS
      expect(response.code).to eq('201')
    end

    it "#show" do
      get "/api/v1/stores/#{store1.id}",  {}, API_V1_HEADERS
      expect(response.code).to eq('200')
      expect(json[:title]).to eq('title1')
    end

    it "#update" do
      put "/api/v1/stores/#{store1.id}", {store: {title: 'title', city: 'city', street: 'street', spaces_count: 1}}.to_json, API_V1_HEADERS
      expect(response.code).to eq('200')
    ...
    it "#delete the store" do
      expect{
         delete "/api/v1/stores/#{store1.id}", {}, API_V1_HEADERS
